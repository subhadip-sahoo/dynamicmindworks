	...
	function alm_query_posts() {
      ...
      if(!empty($meta_key)){
	   ...
	   if(!empty($day)){
		...
		//echo '<pre>';
		//echo($alm_query->query['post_type'][0]);
		// Run the loop
		if ($alm_query->have_posts()) :
         ...
         // If is Unlimited add-ons
			...
			{				
				if($alm_query->query['post_type'][0] == 'testimonials'){
					$include = plugin_dir_path( __FILE__ ) . 'core/repeater/default.php';
				}elseif($alm_query->query['post_type'][0] == 'videos'){
					$include = plugin_dir_path( __FILE__ ) . 'core/repeater/default1.php';
				}
			}				
